...
export default class ProductListSection extends LightningElement
{
	...
	retrievedProductConfigs ({data, error}) {
		if (data) {
			...
			data.records.records.forEach((currentConfig, index)=>{
				...
				currentConfig.fields.Type__c.value, currentConfig.fields.Picklist_Values__c.value));
			});
		} else {
			//handle error
		}
	...
	configurateConfigWrapper(index, configName, configType, dropDownValues) {
		let inputType;
		let isSelect;
		let isNumber;
		let values;
